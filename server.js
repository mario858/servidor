...
app.post('/', function (req, res) {
    ...
    var clave = req.body.inClave.trim();
	 console.log("post received: %s %s", usuario, clave);
    ...
    if (usuario == 'admin' && clave == 'admin') {
        ...
        res.render('index.html', { mensaje: "Usuario o contraseña incorrecta", usuario: usuario });
    }
});
...
app.get('/eliminar', function (req, res) {
    ...
    ps.kill( idProceso, function( err ) {
        if (err) {
            ...
            console.log( 'Process %s  killed!', idProceso);
            res.redirect('/detalle');
        }
    });
});
...
app.get('/memoria', function(req, res){
	...
	var consumida = os.totalmem() - os.freemem();
	var porcentaje = os.freememPercentage();
    console.log("Get memoria total consumida porcentaje: %s , %s , %s", total, consumida, porcentaje);
	res.render("memoria.html", {memconsumida: consumida, memtotal: total , memporcentaje: porcentaje});
